...
3. **Multifactor**: Call **`strategy_factor_analysis`** with a `series_json` object mapping each fetch key (e.g. `bitcoin`, `ethereum`, `SOL`) to that response’s JSON body. Set `trade_type` (`buy_with_usdc`, `swap_asset`, `rebalance_asset`, …), `risk_tier`, and deployable fields per STRATEGY.md.
